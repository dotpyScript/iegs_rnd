...
const QuickStatsWidget = () => {
  ...
  const recentUpdates = [
    { id: 1, title: 'Project Alpha completed', time: '5m ago', type: 'success' },
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-2xl p-5 shadow-md border-2 overflow-hidden min-h-[145px] flex flex-col
        ${isDarkMode 
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        <Activity className={`w-4 h-4 ${isDarkMode ? 'text-blue-600' : 'text-black'}`} />
      ...
      <div className="flex-1 overflow-y-auto space-y-1.5 scrollbar-thin pr-1">
        {recentUpdates.map((update, index) => {
          ...
          return (
            ...
            >
              <div className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 ${getIconColor(update.type)}`}>
                <Icon className="w-2 h-2" />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-[11px] font-medium mb-0.5 line-clamp-1
                  ${isDarkMode ? 'text-gray-900' : 'text-black'}
                `}>
                  {update.title}
                </p>
